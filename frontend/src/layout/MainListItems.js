...
const MainListItems = (props) => {
  ...
  const [tags, dispatch2] = useReducer(reducer2, []);
  ...
  const fetchTags = useCallback(async () => {
    try {
      ...
      dispatch2({ type: "LOAD_TAGS", payload: data.tags });
      ...
      <Can
        ...
        color="inherit">
          ...
          <>
            ...
            {tags.map((tag) => (
              <ListItemLink
              to={"/tickettagged/".concat(tag.id)}
              primary={tag.name}
              icon={<LocalOfferIcon style={{color: tag.color}} />}
              />
              ))}
            <Divider/>
            <ListItemLink
